...
class Graphics:
    ...
    def brain_tumor(self):
        ...
        self.tumor_location = self.screenVR.get_rect().center
        self.tumor_rect = pygame.Rect(*self.tumor_location, 0, 0).inflate(self.tumor_width, self.tumor_height)

        self.tumor_image = pygame.image.load('brain-tumor.jpg')
        self.tumor_image = pygame.transform.scale(self.tumor_image, (self.tumor_width, self.tumor_height))

        self.screenVR.blit(self.tumor_image, self.tumor_rect.topleft)

        keys = pygame.key.get_pressed()
        if keys[pygame.K_SPACE] and self.haptic.colliderect(self.tumor_rect):
            self.snake_mode = False  # Change gripper mode
            self.tumor_visible = False  # Stop drawing the tumor
    ...
    def snake_gripper(self):
        ...
        self.snake_image = pygame.transform.scale(self.snake_image, (self.haptic_width, self.haptic_height))

        if hasattr(self, 'snake_image'):
            self.screenVR.blit(self.snake_image, self.haptic.topleft)
        else:
